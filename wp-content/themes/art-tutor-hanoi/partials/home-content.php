...
$gallery_batches   = array( 1 => array(), 2 => array() );
foreach ( $frontpage_gallery as $item ) {
	$batch = (int) ( $item['batch'] ?? 1 );
	if ( ! isset( $gallery_batches[ $batch ] ) ) {
		$gallery_batches[ $batch ] = array();
	}
	$gallery_batches[ $batch ][] = $item;
}
?>
<!-- Hero -->
      ...
      <?php foreach ( $gallery_batches as $batch_num => $batch_items ) : ?>
        ...
        <div class="gallery-batch gallery-collage<?php echo $batch_num === 1 ? ' is-visible' : ''; ?>">
          <?php foreach ( $batch_items as $item ) : ?>
            <?php
            $img_src  = cld_frontpage_image( $item, (int) ( $item['display_width'] ?? 400 ) );
            $img_size = cld_frontpage_display_size( $item, (int) ( $item['display_width'] ?? 400 ) );
            $style    = sprintf(
            ...
            ?>
            <figure
              class="gallery-item<?php echo ! empty( $item['badge_near'] ) ? ' gallery-item--badge' : ''; ?>"
          ...
          <?php endforeach; ?>
          <?php if ( $batch_num === 1 ) : ?>
            <div class="gallery-badge" aria-hidden="false">
              Fine art
              <span class="gallery-badge__year">since 2021</span>
            </div>
          <?php endif; ?>
        </div>
      <?php endforeach; ?>

      <button type="button" class="gallery-more">Load more photos</button>
    </div>
  </section>
