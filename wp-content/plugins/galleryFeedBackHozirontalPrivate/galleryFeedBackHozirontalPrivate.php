<?php
/*
Plugin Name: GalleryFeedBackPrivate
Kế thừa gallery feedback free
Version: 1.0
...
add_shortcode('galleryFeedBackPrivate', 'galleryFeedBackPrivate_shortcode');

function galleryFeedBackPrivate_shortcode($atts) {
    ...
    $atts = shortcode_atts([
        ...
        //'displayName_column'     => 'Display Your Name on Gallery?', Bỏ
        'teacherComment_column'  => 'Teacher comment',
        'teacherImage_column'   => 'Teacher Image Upload',
        'start_row'              => 2,
        'items_per_page'         => 9,
    ], $atts, 'galleryFeedBackPrivate');
...
if (empty($csv)) {
    ...
    $image_idx        = array_search($atts['image_column'], $headers);
    $email_idx        = array_search($atts['email_column'], $headers);
    ...
    //$displayName_idx    = array_search($atts['displayName_column'], $headers); Bỏ
    ...
    foreach ($rows as $row) {
        ...
        //if (empty($row[$image_idx]) || empty($row[$teacherComment_idx])) continue;
        //Không có link ảnh thì next
        if (empty($row[$image_idx])) continue;
        
        ////////////////////////////////////////////
        //Lọc email ở đoạn này !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!       
        $current_user = wp_get_current_user();
        if (($current_user->user_email) !=($row[$email_idx]) ){ 
    ...
    //Cut 1 phần nhỏ để hiện phân trang page_rows
...
foreach ($paged_rows as $row) {
    $commentState = ' '; //Default là chưa comment
    $raw_url     = trim($row[$image_idx]);
    $opt_url     = str_replace(
                      '/upload/',
    ...
    $commentState = ' '; // Default là chưa comment
    //Nếu có comment thì state đổi :
    if (!empty($row[$teacherComment_idx])) $commentState = '&#x2705; Commented';
    else $row[$teacherComment_idx]='We are discussing your artwork. Please wait !';
    ...
    $html .= '<div style="break-inside: avoid; margin-bottom:20px; text-align:center;">'
          ...
          .  'onclick="galleryFeedBackPrivate_openLightbox(this)" '
          ...
          .     '<div class="caption-author"><strong>' . $commentState . '</strong></div>'
...
<div id="lightbox" style="width:100%; height:100%;">
  <div id="lightbox-overlay" onclick="galleryFeedBack_closeLightbox()" style="
         ...
         z-index: 10000;
        ">
    <div style="display:flex; flex-direction:column; border-radius:0px; max-width:90vw; max-height:90vh;">
       <div style="flex:1;">
...
<script>
function galleryFeedBackPrivate_openLightbox(img) {
  ...
  lightImg.onload = function() {
    //authorEl.innerHTML   = "<strong></strong>" + (img.dataset.author || "N/A"); Tạm thời cần hiện tên private
  ...
  function galleryFeedBackPrivate_closeLightbox() {
